...
class DFRobot_Environmental_Sensor:
    ...
    def __init__(self, bus, baud=9600):
        if bus != 0:
            ...
            self.master = modbus_rtu.RtuMaster(
                serial.Serial(
                    port="/dev/ttyAMA0", baudrate=baud, bytesize=8, parity="N", stopbits=1
                )
            )
    ...
    def get_luminousintensity(self):
        ...
        if self._uart_i2c == I2C_MODE:
            ...
            data = rbuf[0]
        luminous = data * (
            1.0023 + data * (8.1488e-5 + data * (-9.3924e-9 + data * 6.0135e-13))
        )
...
class DFRobot_Environmental_Sensor_UART(DFRobot_Environmental_Sensor):
    ...
    def _read_reg(self, reg_addr, length):
        ...
        """
        return list(
            self.master.execute(self._addr, cst.READ_INPUT_REGISTERS, reg_addr / 2, length / 2)
        )
